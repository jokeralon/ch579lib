  ...
  for (;;) {
    ...
    while (USB_RequestCountI != USB_RequestCountO) {
      ...
      // Execute DAP Command (process request and prepare response)
			printf("DAP_ExecuteCommand, USB_RequestIndexO %d, USB_ResponseIndexI %d\r\n", USB_RequestIndexO, USB_ResponseIndexI);
			printf("USB_Request: ");
			for(int i=0; i< USBD_CUSTOMHID_OUTREPORT_BUF_SIZE; i++)
			{
				printf(" %02X", USB_Request[0][i]);
			}
			printf("\r\n");
			printf("USB_Response: ");
			for(int i=0; i< USBD_CUSTOMHID_OUTREPORT_BUF_SIZE; i++)
			{
				printf(" %02X", USB_Response[0][i]);
			}
			printf("\r\n");
...
void DAP_Task_Init()
{
	...
	tmos_start_task(DAP_Task_ID, 1, 100);
}
