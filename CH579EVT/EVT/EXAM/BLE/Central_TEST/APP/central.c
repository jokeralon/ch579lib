...
// Peer device address
static uint8 PeerAddrDef[B_ADDR_LEN] = { 0x61,0x46,0xB1,0xE4,0xC2,0x84 };
...
// Value to write
static uint8 centralCharVal = 0x01;
...
uint16 Central_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {   
		usb_printf("START_READ_OR_WRITE_EVT\r\n");
  ...
  {   
		usb_printf("START_WRITE_CCCD_EVT\r\n");
    if( centralProcedureInProgress == FALSE )
    {
			// Do a write
			attWriteReq_t req;
...
static void centralProcessGATTMsg( gattMsgEvent_t *pMsg )
{
         ...
         ( pMsg->msg.errorRsp.reqOpcode == ATT_EXCHANGE_MTU_REQ ) ) )
  {
    if ( pMsg->method == ATT_ERROR_RSP )
    {
      ...
      usb_printf( "Exchange MTU Error: %x\r\n", status );
    }
         ...
         ( pMsg->msg.errorRsp.reqOpcode == ATT_READ_REQ ) ) )
  {
    if ( pMsg->method == ATT_ERROR_RSP )
    {
      ...
      usb_printf( "Read Error: %x\r\n", status );
    }
    else
    {
      // After a successful read, display the read value
      usb_printf("Read rsp: %x\r\n",*pMsg->msg.readRsp.pValue);
    }
    centralProcedureInProgress = FALSE;
  }
         ...
         ( pMsg->msg.errorRsp.reqOpcode == ATT_WRITE_REQ ) ) )
  {
    ...
    if ( pMsg->method == ATT_ERROR_RSP )
    {
      ...
      usb_printf( "Write Error: %x\r\n", status );
    }
    else
    {
      // Write success
      usb_printf( "Write success \r\n" );      
    }
...
static void centralEventCB( gapRoleEvent_t *pEvent )
{
  switch ( pEvent->gap.opcode )
  {
    case GAP_DEVICE_INIT_DONE_EVENT:  
      {
				usb_printf("GAP_DEVICE_INIT_DONE_EVENT\r\r\n");
        usb_printf( "Discovering...\r\n" );
    ...
    case GAP_DEVICE_INFO_EVENT:
      {
        // Add device to list
//				usb_printf("GAP_DEVICE_INFO_EVENT\r\r\n");
      ...
      case GAP_DEVICE_DISCOVERY_EVENT:
      {
        ...
        uint8 i;        
				usb_printf("GAP_DEVICE_DISCOVERY_EVENT\r\r\n");
    ...
    case GAP_LINK_ESTABLISHED_EVENT:
      {
        ...
        if ( pEvent->gap.hdr.status == SUCCESS )
        {
          ...
          usb_printf( "%d Connected...\r\n", __LINE__ );
        }
        else
        {
          usb_printf( "Connect Failed...Reason:%X\r\n",pEvent->gap.hdr.status );
					usb_printf( "Discovering...\r\n" );
          centralScanRes = 0;
					GAPRole_CentralStartDiscovery( DEFAULT_DISCOVERY_MODE,
																 DEFAULT_DISCOVERY_ACTIVE_SCAN,
    ...
    case GAP_LINK_TERMINATED_EVENT:
      {
        ...
        usb_printf( "Disconnected...Reason:%x\r\n", pEvent->linkTerminate.reason );
        usb_printf( "Discovering...\r\n" );
        GAPRole_CentralStartDiscovery( DEFAULT_DISCOVERY_MODE,
                               DEFAULT_DISCOVERY_ACTIVE_SCAN,
                               DEFAULT_DISCOVERY_WHITE_LIST ); 
...
static void centralPairStateCB( uint16 connHandle, uint8 state, uint8 status )
{
  ...
  {
    usb_printf( "Pairing started:%d\r\n", status);
  }
  else if ( state == GAPBOND_PAIRING_STATE_COMPLETE )
  {
    if ( status == SUCCESS )
    {
      usb_printf( "Pairing success\r\n" );
    }
    else
    {
      usb_printf( "Pairing fail\r\n" );
    }
  }
  ...
  else if (state == GAPBOND_PAIRING_STATE_BOND_SAVED)
  {
    if (status == SUCCESS)
    {
      usb_printf("Bond save success\r\n");
    }
    else
    {
      usb_printf("Bond save failed: %d\r\n", status);
    }
  }
}
...
static void centralStartDiscovery( void )
{
  ...
  // Discovery simple BLE service
	usb_printf("GATT_DiscPrimaryServiceByUUID\r\n ");
																	 usb_printf("centralConnHandle:%2X\r\n", centralConnHandle);
																	 usb_printf("UUID: ");
																	 for(int i=0;i< ATT_BT_UUID_SIZE;i++)
																		usb_printf("%2X ", uuid[i]);
																	 usb_printf("\r\n");
																	 usb_printf("ATT_BT_UUID_SIZE:%d\r\n", ATT_BT_UUID_SIZE);
																	 usb_printf("centralTaskId:%d\r\n", centralTaskId);
...
static void centralGATTDiscoveryEvent( gattMsgEvent_t *pMsg )
{
  ...
  usb_printf("centralGATTDiscoveryEvent\r\n");
  if ( centralDiscState == BLE_DISC_STATE_SVC )
  {
		usb_printf("BLE_DISC_STATE_SVC\r\n");
         ...
         pMsg->msg.findByTypeValueRsp.numInfo > 0 )
    {
      ...
      usb_printf("Found Profile Service handle : %x ~ %x \r\n",centralSvcStartHdl,centralSvcEndHdl);
  ...
  {
		usb_printf("BLE_DISC_STATE_CHAR\r\n");
         ...
         pMsg->msg.readByTypeRsp.numPairs > 0 )
    {
      centralCharHdl = BUILD_UINT16( pMsg->msg.readByTypeRsp.pDataList[0],
			...
			usb_printf("%2X %2X \r\n",pMsg->msg.readByTypeRsp.pDataList[0], pMsg->msg.readByTypeRsp.pDataList[1]);    
      // Start do read or write
      tmos_start_task( centralTaskId, START_READ_OR_WRITE_EVT, DEFAULT_READ_OR_WRITE_DELAY);
      
      // Display Characteristic 1 handle
      usb_printf("Found Characteristic 1 handle : %x \r\n",centralCharHdl);      
    }
    if ( ( pMsg->method == ATT_READ_BY_TYPE_RSP  && 
           pMsg->hdr.status == bleProcedureComplete ) ||
         ( pMsg->method == ATT_ERROR_RSP ) )
		{
			usb_printf("bleProcedureComplete\r\n");
			// Discover characteristic
  ...
  {
		usb_printf("BLE_DISC_STATE_CCCD\r\n");
    // Characteristic found, store handle
    if ( pMsg->method == ATT_READ_BY_TYPE_RSP && 
         pMsg->msg.readByTypeRsp.numPairs > 0 )
    {
      ...
      // Display Characteristic 1 handle
      usb_printf("Found client characteristic configuration handle : %x \r\n",centralCCCDHdl);      
    }
    centralDiscState = BLE_DISC_STATE_IDLE;
  }    
}
...
static void centralAddDeviceInfo( uint8 *pAddr, uint8 addrType )
{
  ...
  if ( centralScanRes < DEFAULT_MAX_SCAN_RES )
  {
    ...
    centralScanRes++;
    // Display device addr
    usb_printf("Device %d - Addr %x %x %x %x %x %x \r\n",centralScanRes,
