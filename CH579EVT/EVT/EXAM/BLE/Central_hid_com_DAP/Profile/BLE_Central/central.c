...
static void centralProcessGATTMsg( gattMsgEvent_t *pMsg )
{
         ...
         ( pMsg->msg.errorRsp.reqOpcode == ATT_WRITE_REQ ) ) )
  {
  ...
  else if ( pMsg->method == ATT_HANDLE_VALUE_NOTI )
	{
		...
		USBD_HID0_SetReport(HID_REPORT_OUTPUT, 0, 0, pMsg->msg.handleValueNoti.pValue, pMsg->msg.handleValueNoti.len);
	}
...
void BlE_Central_SendData(uint8_t *data, uint8_t len)
{
		attWriteReq_t req;
