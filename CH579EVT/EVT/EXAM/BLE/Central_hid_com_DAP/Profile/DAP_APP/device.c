...
void DAP_LED_Init(void)
{
	 ...
	 {
			GPIOA_ModeCfg(LED_CONNECTED_Pin, GPIO_ModeOut_PP_5mA);
			GPIOA_ModeCfg(LED_RUNNIN_Pin, GPIO_ModeOut_PP_5mA);
		  GPIOA_ResetBits(LED_CONNECTED_Pin);
		  GPIOA_ResetBits(LED_RUNNIN_Pin);
	 }else if( LED_GPIO_Port == GPIOB )
	 {
			GPIOB_ModeCfg(LED_CONNECTED_Pin, GPIO_ModeOut_PP_5mA);
			GPIOB_ModeCfg(LED_RUNNIN_Pin, GPIO_ModeOut_PP_5mA);
		  GPIOB_ResetBits(LED_CONNECTED_Pin);
		  GPIOB_ResetBits(LED_RUNNIN_Pin);
	 }
